...
- Сервер мессенджера отправляет сообщение пользователю B и сохраняет сообщение в базе данных, если статус получателя не в сети.
- Клиент Б отправляет подтверждение на сервер мессенджера.
- Сервер мессенджера уведомляет клиента А об успешной доставке сообщения.
- Когда Клиент Б читает сообщение, приложение уведомляет об этом сервер мессенджера.
- Сервер мессенджера уведомляет клиента А о том, что Клиент Б прочитал сообщение.
